...
class WhatsNext
{
		...
		while(true)
		{
			...
			else
			{
				...
				if(isAnAp(split))
				{
					...
					int ap = Integer.parseInt(split[1])+len*diff;
					System.out.println("AP "+ap);
				}
				else if(isinGp(split))
				{
					int ratio = Integer.parseInt(split[1])/Integer.parseInt(split[0]);
					int gp = Integer.parseInt(split[1])*(int)Math.pow(ratio,len);
	...
	public static boolean isAnAp(String a[])
	{
		long diff = Integer.parseInt(a[1])-Integer.parseInt(a[0]);
		for(int i=0;i<a.length-1;i++)
		{
			if((Integer.parseInt(a[i+1])-Integer.parseInt(a[i])) != diff)
				return false;
		}
		return true;
	}
	
	public static boolean isinGp(String a[])
	{
		int ratio = Integer.parseInt(a[1])/Integer.parseInt(a[0]);
		for(int i=0;i<a.length-1;i++)
		{
			if((Integer.parseInt(a[i+1])/Integer.parseInt(a[i])) != ratio)
				return false;
		}
		return true;
	}
